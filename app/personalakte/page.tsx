    ...
    <main style={{ fontFamily: "'Atkinson Hyperlegible', sans-serif", background: '#f5f5f5', minHeight: '100vh', color: '#1a1a1a' }}>
      ...
      <section style={{ padding: '5rem 1.75rem 3rem', textAlign: 'center', maxWidth: '860px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: '3rem', fontWeight: 700, lineHeight: 1.1, marginBottom: '1rem', color: '#1a1a1a' }}>
          Digitale Personalakte – <span style={{ color: '#F97316' }}>sicher & jederzeit abrufbar</span>
        </h1>
        <p style={{ fontSize: '1.15rem', color: '#666', lineHeight: 1.7, marginBottom: '2rem' }}>
          Schluss mit Papierchaos und Aktenschränken. Alle Personaldokumente sicher digital – DSGVO-konform, strukturiert und jederzeit verfügbar.
        </p>
      ...
      <section className="section" style={{ padding: '3rem 1.75rem', maxWidth: '860px', margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '1rem', color: '#555', lineHeight: 1.8 }}>
          Die digitale Personalakte ersetzt den klassischen Aktenschrank. Alle relevanten Dokumente eines Mitarbeiters – von Arbeitsvertrag über Lohnabrechnungen bis zu Bescheinigungen – werden sicher digital gespeichert, strukturiert abgelegt und sind jederzeit abrufbar. DSGVO-konform, verschlüsselt und revisionssicher.
        </p>
      </section>
      <section style={{ background: '#fff', padding: '3rem 1.75rem', margin: '2rem 0' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div className="grid3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            {[
              { titel: 'Dokumentenverwaltung', text: 'Alle Personaldokumente strukturiert abgelegt – Verträge, Bescheinigungen, Zeugnisse.' },
              { titel: 'Lohnabrechnungen', text: 'Monatliche Abrechnungen automatisch in der Akte hinterlegt – immer vollständig.' },
              { titel: 'Mitarbeiterzugang', text: 'Jeder Mitarbeiter sieht nur seine eigenen Dokumente – sicher und datenschutzkonform.' },
              { titel: 'DSGVO-konform', text: 'Verschlüsselte Speicherung in Deutschland – alle gesetzlichen Anforderungen erfüllt.' },
              { titel: 'Revisionssicher', text: 'Dokumente können nicht unbemerkt geändert werden – wichtig bei Prüfungen.' },
              { titel: 'Papierlos', text: 'Kein Drucker, kein Aktenschrank, kein Ablageaufwand – alles digital.' },
            ].map((item) => (
      ...
      <section className="section" style={{ padding: '3rem 1.75rem', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '1.5rem', color: '#1a1a1a' }}>Für wen ist die digitale Personalakte sinnvoll?</h2>
        <div className="grid2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1rem' }}>
          {[
            { titel: 'Unternehmen ab 5 Mitarbeitern', text: 'Ab dieser Größe lohnt sich eine strukturierte Personalverwaltung.' },
            { titel: 'Mehrere Standorte', text: 'Alle Dokumente zentral – egal von wo zugegriffen wird.' },
            { titel: 'Homeoffice & Remote', text: 'Kein Büro nötig – Dokumente sind von überall erreichbar.' },
            { titel: 'Wachsende Teams', text: 'Neue Mitarbeiter sofort eingebunden – Onboarding digital und effizient.' },
          ].map((item) => (
      ...
      <section style={{ background: '#fff', padding: '3rem 1.75rem', margin: '2rem 0' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div className="grid3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem' }}>
            {[
              { titel: 'Alles integriert', text: 'Abrechnung und Personalakte aus einer Hand – keine Schnittstellen nötig.' },
              { titel: 'Sofort startklar', text: 'Kein aufwändiges Setup – wir richten alles ein.' },
              { titel: 'DSGVO-sicher', text: 'Speicherung in Deutschland, verschlüsselt und revisionssicher.' },
              { titel: 'Kein Papierchaos', text: 'Dokumente digital ablegen spart Zeit und Platz.' },
              { titel: 'Mitarbeiterzugang', text: 'Jeder sieht seine eigenen Dokumente – transparent und fair.' },
              { titel: 'Fester Ansprechpartner', text: 'Bei Fragen sind wir direkt erreichbar – kein Callcenter.' },
            ].map((item) => (
